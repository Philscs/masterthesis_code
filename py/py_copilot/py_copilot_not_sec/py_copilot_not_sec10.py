...
class ChatServer:
    ...
    def store_offline_message(self, username: str, message: Message):
        self.offline_messages.setdefault(username, []).append(message)
        self.db.store_message(message)

    async def send_private_message(self, message: Message):
        recipient = self.clients.get(message.recipient)
        if recipient:
            await recipient.send_message(message)
        else:
            self.store_offline_message(message.recipient, message)
    ...
    def encrypt_message(self, content: str) -> bytes:
        ...
        room_key = self.message_keys.setdefault(self.username, Fernet.generate_key())
        f = Fernet(room_key)
...
class ClientConnection:
    ...
    async def handle_offline_messages(self):
        messages = self.server.offline_messages.pop(self.username, [])
