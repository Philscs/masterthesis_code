...
class CodeSmellDetector:
    ...
    def extract_code_smells(self):
        ...
        self._extract_duplicate_code_smells(smills)
        return smells
    ...
    def suggest_refactoring(self):
        ...
        for smell in self.smells:
            if smell == 'Long Method':
                suggestion = f'Kürzen der Methode {random.randint(1, 5)} Zeilen'
                suggestions.append(suggestion)
            elif smell == 'Dead Code':
                suggestion = f'Removal von unnötigen Code'
                suggestions.append(suggestion)
            elif smell == 'Duplicate Code':
                suggestion = f'Stellen Sie sicher, dass alle gleichen Code-Blöcke einzeln 
